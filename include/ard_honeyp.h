...
inline int ard_honeyp_read(uint16_t *data) {
    ...
    /* copy to data and bit-shift to 14-bits */
    bytes_in[0] &= 0x3f;
    memcpy(data, bytes_in, sizeof(uint16_t));

    // bool s1 = bytes_in[1] >> 7; // ((bytes_in[1] & ARD_HONEYP_BIT_7) == ARD_HONEYP_BIT_7);
    // bool s0 = bytes_in[1] >> 6; // ((bytes_in[1] & ARD_HONEYP_BIT_6) == ARD_HONEYP_BIT_6);
    if (s == 0) {
        /* new data */
        return 0;
    } else if (s == 2) {
